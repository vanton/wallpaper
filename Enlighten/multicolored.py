...
def run_tests(manager, tests=100):
    ...
    bar_format = (
        ...
        + "S:"
        + terminal.green3("{count_0:{len_total}d}")
        + " "
        + "F:"
        + terminal.red2("{count_2:{len_total}d}")
        + " "
        + "E:"
        + terminal.yellow2("{count_1:{len_total}d}")
        + " "
        + "[{elapsed}<{eta}, {rate:.2f}{unit_pad}{unit}/s]"
    )

    with manager.counter(
        total=tests, desc="Testing", unit="tests", color="green3", bar_format=bar_format
    ) as success:
...
def load(manager, units=80):
    ...
    pb_connecting = manager.counter(
        total=units, desc="Loading", unit="services", color="red2", bar_format=BAR_FMT
    )
