...
def process_files(manager):
    ...
    enterprise = manager.counter(
        total=DATACENTERS, desc="Processing:", unit="datacenters"
    )
    ...
    for d_num in range(1, DATACENTERS + 1):
        ...
        datacenter = manager.counter(
            total=systems, desc="  Datacenter %d:" % d_num, unit="systems", leave=False
        )
        ...
        for s_num in range(1, systems + 1):
            ...
            system = manager.counter(
                desc="    System %d:" % s_num, unit="files", leave=False
            )
            ...
            # Log status
            LOGGER.info(
                "Updated %d files on System %d in Datacenter %d", files, s_num, d_num
            )
